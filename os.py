...
async def ai_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # FIX 1: Dùng asyncio.to_thread với hàm đồng bộ (generate_content) 
        # Cách này ổn định 100% trên Railway/Linux, không bị lỗi gRPC
        response = await asyncio.to_thread(ai_model.generate_content, prompt)
        
        # FIX 2: Bỏ parse_mode="Markdown" HOẶC đổi thành parse_mode=None
        # Vì Gemini hay sinh ra ký tự đặc biệt (*, _) làm Telegram crash lỗi "Can't parse entities"
        await update.message.reply_text(f"🤖 AI:\n{response.text}")
        
    except Exception as e: 
        # FIX 3: In thẳng lỗi thực sự ra chat để bạn biết đường sửa
        error_msg = str(e)
        logging.error(f"Lỗi AI chi tiết: {error_msg}")
        
        if "API_KEY_INVALID" in error_msg or "400" in error_msg:
            await update.message.reply_text("❌ Lỗi: API Key Gemini của Ông chủ không hợp lệ hoặc chưa được cài đặt!")
        else:
            await update.message.reply_text(f"❌ Lỗi chi tiết:\n`{error_msg[:100]}...`", parse_mode="Markdown")
