...
# Chỉ thị ngầm ép AI đóng vai chuẩn xác
system_prompt = (
    "Bạn là một quản gia AI trung thành, thông minh và tinh tế của Ông chủ. "
    "Luôn xưng hô là 'Dạ', 'Ông chủ' và 'tôi' hoặc 'em'. "
    "Trả lời ngắn gọn, đi thẳng vào vấn đề, tuyệt đối không lan man. "
    "Nếu Ông chủ hỏi về các thông tin thời gian thực, hãy dựa vào 'Thông tin hệ thống' được cung cấp."
)

ai_model = genai.GenerativeModel(
    'gemini-2.5-flash', # Hoặc đổi thành 'gemini-1.5-pro' nếu muốn nó thông minh xuất sắc (nhưng chậm hơn 1 chút)
    system_instruction=system_prompt,
    generation_config=genai.types.GenerationConfig(
        temperature=0.4, # Mức 0.4 giúp AI trả lời logic, bớt "ngáo" và nói nhảm
    )
)

# Thêm biến này ở dưới cùng phần khai báo biến để lưu trí nhớ cho AI
chat_sessions = {}
...
# ===== 4. XỬ LÝ AI (GEMINI) - BẢN NÂNG CẤP =====
async def ai_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    query = " ".join(context.args)
    
    if not query:
        await update.message.reply_text("🤖 Dạ, Ông chủ cần em giúp gì ạ?")
        return
        
    await context.bot.send_chat_action(chat_id=chat_id, action="typing")
    
    try:
        # 1. TẠO TRÍ NHỚ (CHAT HISTORY)
        # Nếu là lần đầu chat, tạo một phiên chat mới cho chat_id này
        if chat_id not in chat_sessions:
            chat_sessions[chat_id] = ai_model.start_chat(history=[])
            
        chat = chat_sessions[chat_id]
        
        # 2. BƠM NGỮ CẢNH THỜI GIAN THỰC
        now_str = datetime.now(VN_TZ).strftime("%A, ngày %d/%m/%Y, %H:%M:%S")
        
        # Gói câu hỏi của Ông chủ kèm theo thông tin ngày giờ
        full_query = f"[Thông tin hệ thống: Hiện tại là {now_str}]\nÔng chủ hỏi: {query}"
        
        # 3. GỬI TIN NHẮN (Dùng chat.send_message thay vì generate_content)
        response = await asyncio.to_thread(chat.send_message, full_query)
        
        # Trả lời Ông chủ (bỏ parse_mode để tránh lỗi Markdown như đã giải thích)
        ...
        if "API_KEY_INVALID" in error_msg:
            ...
            await update.message.reply_text("❌ Xin lỗi Ông chủ, hệ thống suy nghĩ của em đang bị lỗi kết nối.")
