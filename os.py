...
# FIX: Ép API-Sports sử dụng múi giờ Việt Nam để không bị lệch ngày
...
DATA_FILE = os.path.join(DATA_DIR, "supreme_v8_data.json")
...
system_prompt = (
    ...
    "3. NHẬN XÉT SÂU SẮC: Phân tích lý do tại sao thắng/thua, yếu tố con người và cảnh báo rủi ro sắc bén dựa trên [Hồ sơ Ông chủ].\n"
    "4. NGẮN GỌN & ĐANH THÉP: Lời lẽ phải uy lực, thông minh."
)
...
def load_data():
    if os.path.exists(DATA_FILE):
        ...
        except Exception as e: logging.error(f"Lỗi đọc file data: {e}")
...
# ===== 3. LỆNH CHUNG & HỌC HỎI =====
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    state["chat_id"] = update.effective_chat.id
    save_data()
    menu = (
        "🤵 **AI COMMANDER V8 - CHUYÊN GIA TỐI THƯỢNG**\n\n"
        "🧠 **[ BỘ NÃO AI ]**\n"
...
# ===== 4. TÌM KIẾM THÔNG MINH & NHIỀU LỰA CHỌN =====
async def matches_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    today = datetime.now(VN_TZ).strftime("%Y-%m-%d")
    kb =[]
    await context.bot.send_chat_action(chat_id=update.effective_chat.id, action="typing")
    try:
        res_f = await client.get(f"https://v3.football.api-sports.io/fixtures?date={today}{TZ_PARAM}")
        ...
        res_n = await client.get(f"https://v2.nba.api-sports.io/games?date={today}{TZ_PARAM}")
        ...
        res_b = await client.get(f"https://v1.basketball.api-sports.io/games?date={today}{TZ_PARAM}")
        for m in res_b.json().get("response",[])[:5]:
    ...
    if not kb: return await update.message.reply_text("📭 Không có sự kiện thể thao nào đáng chú ý hôm nay.")
...
async def search_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_chat_action(chat_id=update.effective_chat.id, action="typing")
    try:
        # TÌM CẢ ĐỘI BÓNG LẪN GIẢI ĐẤU (League)
        res_f = await client.get(f"https://v3.football.api-sports.io/fixtures?date={today}{TZ_PARAM}")
    ...
    if not kb: return await update.message.reply_text("ℹ️ Không tìm thấy đội hoặc giải đấu này trong ngày hôm nay.")
    await update.message.reply_text(f"🔍 KẾT QUẢ CHO '{query.upper()}':", reply_markup=InlineKeyboardMarkup(kb[:20]))
...
async def time_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not kb: return await update.message.reply_text(f"ℹ️ Không có trận khung giờ `{target_time}`.")
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("pk_"):
        ...
        try:
            ...
            # FIX TỐI ƯU MULTI-SELECT: Trả lời query nhanh và gửi 1 tin nhắn mới, không xóa menu cũ.
            await query.answer(f"✅ Pick thành công: {m_data['teams']['home']['name']}", show_alert=False)
            kb = [[InlineKeyboardButton("📝 Thêm ghi chú ngay", callback_data=f"asknote_m_{idx}")]]
            await context.bot.send_message(
                query.message.chat_id, 
                f"✅ **ĐÃ THÊM VÀO BOARD:** {icon} {m_data['teams']['home']['name']} vs {m_data['teams']['away']['name']}", 
        ...
        prompt = (f"Phân tích trận {m.get('icon','⚽')}: {m['home']} vs {m['away']} ({league}).\nLịch sử {m['home']}:\n{home_last}\nLịch sử {m['away']}:\n{away_last}\n"
                  ...
                  "PHẢN BIỆN CHUYÊN SÂU: Nếu bóng đá, đánh giá đội hình, chiến thuật. Nếu bóng rổ/NBA, phân tích matchup tay đôi. Chốt kèo thông minh.")
...
async def morning_briefing(context: ContextTypes.DEFAULT_TYPE):
    ...
    except Exception as e: logging.error(f"Lỗi Morning Briefing: {e}")

    prompt = (
        "Viết BẢN TIN SÁNG LÚC 05:00. Highlight 2-3 trận đáng xem nhất (Bóng đá/NBA) và giải thích ngắn gọn lý do tại sao hấp dẫn.\n\n"
        f"Hồ sơ: {' '.join(state.get('profile',[]))}\nDanh sách trận:\n{chr(10).join(lines)}"
    )
    try:
        if chat_id not in chat_sessions: chat_sessions[chat_id] = ai_model.start_chat(history=[])
...
async def monitor(context: ContextTypes.DEFAULT_TYPE):
    ...
    if today in state["boards"]:
        ...
        # Update Kết Quả Tối Ưu (Chỉ gọi API cho những trận ĐÃ ĐẾN GIỜ ĐÁ)
        ...
        if now.timestamp() - last_api_check >= 600 and live_matches:
            ...
            if has_f:
                try:
                    res = await client.get(f"https://v3.football.api-sports.io/fixtures?date={today}{TZ_PARAM}")
            ...
            if has_n:
                try:
                    res = await client.get(f"https://v2.nba.api-sports.io/games?date={today}{TZ_PARAM}")
            ...
            if has_b:
                try:
                    res = await client.get(f"https://v1.basketball.api-sports.io/games?date={today}{TZ_PARAM}")
...
def main():
    ...
    print("🚀 SUPREME AI COMMANDER V8.0 (TIMEZONE FIXED & MULTI-SELECT) ĐÃ SẴN SÀNG!")
    app.run_polling()
