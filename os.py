...
def parse_match_time(utc_date_str):
    ...
    # API-Sports NBA thường dùng 'Z' ở cuối
...
async def get_match_context(m):
    ...
    if sport == "f":
        ...
        # Với NBA và Bóng rổ, AI 2.5 Flash đã nạp sẵn kho dữ liệu khủng về chiến thuật và player.
        # Báo AI dùng trí tuệ tự có để phân tích matchup thay vì chỉ nhìn tỷ số cũ.
        return league, "Sử dụng kiến thức chuyên gia NBA/Bóng rổ để phân tích cầu thủ và chiến thuật.", "Phân tích Matchup cá nhân."
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    menu = (
        ...
        "🧠 **[ BỘ NÃO AI ]**\n"
        " ├ 💬 *Chat trực tiếp để hỏi lịch, phân tích cầu thủ NBA, chiến thuật*\n"
        " ├ 📥 `/learn [Sở thích]` : Dạy AI nhớ gu\n"
        " └ 📊 `/summary` : Tổng kết cuối ngày\n\n"
        "⚽🏀🌟 **[ THỂ THAO: FOOTBALL & NBA ]**\n"
...
async def morning_briefing(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if chat_id not in chat_sessions: chat_sessions[chat_id] = ai_model.start_chat(history=[])
        response = await asyncio.to_thread(chat_sessions[chat_id].send_message, prompt)
        await context.bot.send_message(chat_id, f"🌅 **BẢN TIN THỂ THAO 05:00 SÁNG**\n\n{response.text}")
    except: pass
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("pk_"):
        ...
        prompt = (f"Phân tích trận {m.get('icon','⚽')}: {m['home']} vs {m['away']} ({league}).\nLịch sử {m['home']}:\n{home_last}\nLịch sử {m['away']}:\n{away_last}\n"
                  f"HỒ SƠ: {' '.join(state.get('profile',[]))}\n"
                  "PHẢN BIỆN CHUYÊN SÂU: Nếu là bóng đá, đánh giá đội hình, chiến thuật. Nếu là bóng rổ/NBA, hãy mang số liệu cầu thủ ngôi sao, matchup tay đôi (ví dụ: trung phong vs vòng ngoài) ra để mổ xẻ. Sau đó chốt kèo thông minh.")
...
async def predict_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Same as ai_predict_ logic
    await detail_cmd(update, context) # Gọi detail thay vì code lặp

# ===== 8. MONITOR TỐI ƯU QUOTA =====
async def monitor(context: ContextTypes.DEFAULT_TYPE):
    ...
    # 1. Nhắc 15 phút (Offline - 1 phút chạy 1 lần)
    ...
    if today in state["boards"]:
        ...
        # 2. Update Kết Quả (Chỉ gọi API nếu CÓ trận chưa xong, 10 phút/lần)
        if now.timestamp() - last_api_check >= 600 and unnotified:
            last_api_check = now.timestamp()
            has_f = any(m.get("sport") == "f" for m in unnotified)
            has_n = any(m.get("sport") == "n" for m in unnotified)
            ...
            if has_f:
                try:
                    ...
                    for m in state["boards"][today]:
                        if m.get("sport") == "f" and not m.get("notified") and m["id"] in f_map:
                            f_data = f_map[m["id"]]
            ...
            if has_n:
                try:
                    ...
                    for m in state["boards"][today]:
                        if m.get("sport") == "n" and not m.get("notified") and m["id"] in n_map:
                            n_data = n_map[m["id"]]
                            if n_data["status"]["short"] == 3: # 3 = Finished
...
def main():
    ...
    print("🚀 SUPREME AI COMMANDER V7.0 (FOOTBALL + NBA + MORNING BRIEFING) ĐÃ SẴN SÀNG!")
    app.run_polling()
